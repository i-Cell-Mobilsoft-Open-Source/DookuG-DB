...
--changeset tibor.ivan:0002 labels:0.3.0 dbms:oracle
--comment VDRDEV-504 Creating INTERVAL_DECLARATION_DATA table

. Postgresql liquibase header SQL version:
[source,sql]
--liquibase formatted sql
--changeset tibor.ivan:0002 labels:0.3.0 dbms:postgresql
--comment VDRDEV-504 Creating INTERVAL_DECLARATION_DATA table
...
<?xml version="1.0" encoding="UTF-8"?>
    ...
    <changeSet id="0005" author="jozsef.holczer" labels="0.1.0">
        <comment>ICMV2-44-Creating TAXI_INVOICE table, Foreign Key index.</comment>
        <!-- FK Index -->
