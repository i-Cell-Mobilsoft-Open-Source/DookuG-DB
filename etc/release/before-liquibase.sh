...
if [[ "$AUTO_INSTALL" == "postgresql" ]]; then
  #in case of local development, you don't need to fill the INSTALL_PGTOOLS, only if you do not need the partition manager, in this case give this param with false.
  INSTALL_PGTOOLS=${INSTALL_PGTOOLS:-true}
  export INSTALL_PGTOOLS
  echo "INSTALL_PGTOOLS=$INSTALL_PGTOOLS"
fi  

#in case of local development, you don't need to fill the INSTALL_... params.
if [[ "$AUTO_INSTALL" == "postgresql" ]]; then
  ...
  for step in "${STEPS[@]}"; do
    case $step in
      1)
        ...
        echo "  INSTALL_PASSWORD_PROJECT=$INSTALL_PASSWORD_PROJECT"
      ...
      2)
        if [[ "$INSTALL_PGTOOLS" == true ]]; then         
          ...
          # in case you need a debug run: --logLevel=debug \
          # --default-schema-name=public: The PG_TOOLS is an external image, so we need to provide where it finds the databasechangelog table.
