...
if [[ "$AUTO_INSTALL" == "postgresql" ]]; then
  #in case of local development, you don't need to fill the INSTALL_PGTOOLS, but if you don't need it, use 0.
  INSTALL_PGTOOLS=${INSTALL_PGTOOLS:-1}
  export INSTALL_PGTOOLS
  echo "INSTALL_PGTOOLS=$INSTALL_PGTOOLS"

  #in case of local development, you don't need to fill the INSTALL_.. params., b/c they can use default values.
      ...
      case $step in
        ...
        2)
          if [[ "$INSTALL_PGTOOLS" == "1" ]]; then         
            ...
            # in case of debug needed, insert it right before the update: --logLevel=debug \
            liquibase --searchPath=../${DOCKER_REPOSITORY}/liquibase/changelog \
                ...
                --default-schema-name=${S2_SCHEMA_NAME} \
