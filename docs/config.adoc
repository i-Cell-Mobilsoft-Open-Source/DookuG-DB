...
* ENV variables:
  ...
  - **DBDWH_IMAGE_VERSION**: used version of liquibase, postgres, partman images (_default value: 0.9.0_)
  - **VERSION**: Project version, this is set automatically.
* Compose variables:
  - **LIQUIBASE_BASE_IMAGE**: central, corporate base liquibase image with version, used by dockerfile (_default value: DOCKER_REPOSITORY/db-dwh/liquibase:${DBDWH_IMAGE_VERSION}_).
